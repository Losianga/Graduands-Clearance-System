...
if(isset($_POST['submit'])){
	...
	if(!empty($name) || !empty($regNo) || !empty($contact) || !empty($email) || !empty($gender)){
		$sql="INSERT INTO admin_request(name,regNo,contact,email,gender) VALUES('$name','$regNo','$contact','$email','$gender')";
		if($name==''){
		...
		if($result){
			...
			echo "<script>window.open('AdminRequest.php','_self');</script>";
		}
	}
}
/*if(!$_SESSION['email']){
...
<body>
   ...
   <form action="index.php" method="post">
   	<header class="heads">
   		<img src="image1.jpg">
   		<h1>Student Admistation Clearance</h1>
   	</header><br><br>
   	<div class="cont">
