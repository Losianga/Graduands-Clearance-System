...
if(isset($_POST['submit'])){
	...
	if(!empty($name) || !empty($regNo) || !empty($contact) || !empty($email) || !empty($gender)){
		$sql="INSERT INTO clinic_request(name,regNo,contact,email,gender) VALUES('$name','$regNo','$contact','$email','$gender')";
		...
		if($result){
			...
			echo "<script>window.open('ClinicRequest.php','_self');</script>";
		}
	}
}
/*if(!$_SESSION['email']){
...
<body>
   ...
   <form action="ClinicRequest.php" method="post">
   	<header class="heads">
   		<img src="image1.jpg">
   		<h1>Student Clinic Details</h1>
   	</header><br><br>
   	<div class="cont">
